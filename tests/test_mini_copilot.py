...
class TestMiniCopilot(unittest.TestCase):
    ...
    def test_extract_text_from_url_various(self, mock_session):
        ...
        mock_s.get.return_value.apparent_encoding = 'utf-8'
    ...
    def test_login_flow_success(self, mock_post):
    ...
    @patch("mini_copilot.login.get_device_code")
    @patch("mini_copilot.login.poll_for_access_token")
    @patch("mini_copilot.main.CONFIG_PATH")
    def test_run_login(self, mock_path, mock_poll, mock_device):
        mock_device.return_value = {"device_code": "dc", "interval": 1}
        mock_poll.return_value = "at"
        token = main.run_login()
        self.assertEqual(token, "at")
    ...
    def test_main_commands(self, mock_input, mock_load, mock_get_token, mock_search, mock_chat):
        ...
        mock_input.side_effect = ["/help", "/", "/copy", "/model", "", "/search_provider", "1", "Ask", ".exit"]
